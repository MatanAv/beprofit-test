...
describe('getConstantHeaders', () => {
  ...
  test('returns an array of headers', () => {
    expect(getConstantHeaders()).toEqual(constantHeaders);
  });

  test('returns an array of headers with additions', () => {
    const additions = [
      { key: 'test', title: 'test' },
      { key: 'test2', title: 'test2' }
    ];

    expect(getConstantHeaders(additions)).toEqual([...constantHeaders, ...additions]);
  });
});

describe('getConstantDefaultForm', () => {
  const constantDefaultForm = {
    order_ID: 0,
    name: '',
    total_refunded: 0,
    created_at: '',
  ...
  test('returns an object with default values', () => {
    expect(getConstantDefaultForm()).toEqual(constantDefaultForm);
  });

  test('returns an object with default values with additions', () => {
    const additions = {
      test: '',
      test2: 0
    };

    expect(getConstantDefaultForm(additions)).toEqual({ ...constantDefaultForm, ...additions });
  });
});
